...
- **Why:** Подготовка merge в main: цитируемый репозиторий и воспроизводимость прогонов из conference article.
...
- **Change:** Восстановлен `literature/latex/mmcs-sfedu_thesis7/images/tau_model.tikz` из коммита `ccaf620` (ветка `bayesian_changepoint`); в `conference_article.tex` заменён `\includegraphics{tau_model.png}` на `\input{images/tau_model.tikz}`; в preamble — `tikz` + `arrows.meta,fit`.
- **Why:** Пользователь: `tau_model.png` — не та схема; нужна plate-диаграмма из отчёта `ПономаревМБ 072026 - 1.pdf` (препринт «Определение стресса…»).
- **Result:** Исходник — TikZ (`\resizebox{\textwidth}{!}{\input{images/tau_model.tikz}}`), как в `preprint_seismic_stress.tex` после `ccaf620`. PDF пересобран без ошибок.
- **Artifacts:** `images/tau_model.tikz`, `conference_article.pdf`, `preamble_common.tex`
- **Next:** при желании cherry-pick `ccaf620` в main для синхронизации `preprint_seismic_stress.tex`.

## 2026-08-10 — Plate-схема модели τ в conference article

- **Change:** В `conference_article.tex` (раздел «Методы») добавлен `figure*` с `\includegraphics{tau_model.png}` и подписью plate-схемы; ссылка в тексте модели; синхронизирован `chapters/03_methods.md`.
- **Why:** Пользователь: найти код схемы из диссертации/preprint и вставить в статью.
- **Result:** Исходник — готовый PNG (`literature/latex/mmcs-sfedu_thesis7/images/tau_model.png`, копия в `latex/images/`); TikZ-исходника в репо нет. PDF пересобран (8 стр.), метка `fig:tau-model`.
- **Artifacts:** `literature/conference_article_ru/latex/conference_article.tex`, `conference_article.pdf`
- **Next:** при необходимости обновить TikZ/PNG под \(\mathcal{T}=\{2,\ldots,8\}\) и IIB вместо generic StudentT.

## 2026-08-10 — Mean-фигуры + вставка в LaTeX/Results

- **Change:** `plot_e_tau_mean.py` (mean-only by lik, mean context 2×2, mean+IIB by lik); все pub-фигуры копируются в `latex/images/`; Results в tex/md с `\ref{fig:...}`; PDF пересобран.
- **Why:** Пользователь: собрать mean-гистограммы и вставить в статью.
- **Result:** 4 figure* в Results (screening, mean-only, mean-context, IIB main).
- **Artifacts:** `figures/results/plot_e_tau_mean.py`, `latex/images/fig_*.png`, `chapters/05_results.md`, `conference_article.pdf`
- **Next:** при необходимости сократить число figure* (4 full-width — много для 6 стр.).

## 2026-08-10 — Публикационные гистограммы E[τ] (скрининг / IIB)

- **Change:** Скрипт `figures/results/plot_e_tau_screening.py`: density-гистограммы 2×2, ECDF, IIB-срезы; общие оси, заливка края prior, Cell-палитра, PNG+SVG 450 dpi, data-manifest.
- **Why:** Старые hist_* были exploratory (разные ylim, «Previous run», count, шумные легенды).
- **Result:** `fig_screening_e_tau_hist`, `fig_screening_e_tau_ecdf`, `fig_iib_main_e_tau_hist` (+ копии в `reports/figures/density_safe/pub/`).
- **Artifacts:** `literature/conference_article_ru/figures/{results,data,data-manifest.md}`
- **Next:** Вставить фигуры в `conference_article.tex` / Results.

## 2026-08-10 — Статья: скрининг правдоподобий → основной анализ IIB

- **Change:** Переписаны главы 03–07 и `conference_article.tex`/abstract: критерии отбора range-семейств; Results = скрининг (доля E[τ]≤2.05) + IIB-срез + чувствительность маски; протокол: analysis set = IIB.
- **Why:** Пользователь: оформить результаты; IIB — единственное подходящее range-семейство; не «все равны» и не «остальные неверны».
- **Result:** IIB n=366: E[τ]≈5.48 (мед. 5.60), 0% у τ=2; BC/ZOIB/plain — 7–14% у τ=2; mean-only ≈5.78 без пика. B IIB N=24: 4.82/5.70/7.00. PDF пересобран.
- **Artifacts:** `chapters/03–07_*.md`, `latex/conference_article.{tex,pdf}`, `plan/experiment-protocol.md`, `reports/figures/density_safe/`
- **Next:** PMF `tau_probs` refit; опционально фигуры в tex; BC+normal CSV — не смешивать с основным текстом без сверки.

## 2026-08-10 — Range lik freeze: beta_constrained + normal

- **Change:** После hist E[τ] (ZOIB/IIB → пик у τ=2; mean-only правдоподобен ~5.8) range в сетке = `beta_constrained` + `normal`; `normal` добавлен в `VALID_LIKELIHOODS["range"]`. Smoke + nohup full в новом out-dir.
- **Why:** Boundary-семейства на range неинтерпретируемы; mean показал адекватный апостериор τ.
- **Result:** Smoke OK (n=34): BC mean+range E[τ]≈4.57; range normal≈5.89; mean+range normal≈6.58. Full: PID 590198, `run_output_8day_density_safe_bc_normal/full_search.log`.
- **Artifacts:** `scripts/run_parallel_search_8day_density_safe.py`, `run_output_8day_density_safe_bc_normal/`, `experiment-protocol.md`
- **Next:** Дождаться CSV → hist E[τ] по BC/normal; обновить Primary B / Results.
