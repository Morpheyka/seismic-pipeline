    ...
    *,
    ...
    for event in events:
        ...
        if direction == "before":
            ...
            # Reverse order for post-event window: +window_days, ..., +1.
            window_dates = [
                (dt + timedelta(days=offset)).strftime("%Y_%m_%d")
                for offset in range(window_days, 0, -1)
            ]
...
def _cache_needed_dates(
    ...
    for rat_id, date in sorted(needed):
        ...
        if cache_manager.cache_hypnogram(rat_id, date, source="s3"):
            cached.append((rat_id, date))
            continue
        # Fallback to local WD only when S3 did not provide the hypnogram.
...
def export_rem_profiles_10days_cached_only(
    *,
    ...
    summary_json: str = "samples_10days_summary.json",
    window_days: int = 10,
    # Default matches typical user home mount; HypnogramCacheManagerYt also tries ~/mnt/wd/rat if /mnt/wd/rat is missing.
    local_data_root: str = "/home/ponomattik/mnt/wd/rat",
    local_hypnogram_cache_dir: str = "./hypnogram_cache",
    ...
    rows = _build_10day_inputs(events, window_days=window_days)
        ...
        for group_name, features in group_data.items():
            for feat_name, observed_df in features.items():
                ...
                raise ValueError(
                    ...
                    "Use one of: normal, student_t, lognormal, gamma."
                )
