...
| `fig:mean-only` | `latex/images/fig_mean_only_by_lik.{png,pdf}` | гистограммы \(\mathbb{E}[\tau]\) mean-only | yes |
| `fig:mean-profile` | `latex/images/fig_mean_by_profile.{png,pdf}` | interim gallery A | pending choice |
...
| `fig:diag-iib` | `latex/images/fig_diag_iib_{params,overlay}.{png,pdf,svg}` | IIB range confirmatory: params KDE + median/90% likelihood overlay (`refit_iib_range` 02+06) | yes |
| `fig:diag-mean-nu` | `latex/images/fig_diag_mean_nu_{params,overlay}.{png,pdf,svg}` | mean Student-\(t\) \(\nu\)-split: params KDE + median/90% overlay (`refit_student_t_nu_split` 02+06) | yes |
